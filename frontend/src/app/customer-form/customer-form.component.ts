...
export class CustomerFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.id != -1)
      this.service.retrieveCustomer("test1", this.id).subscribe(
  ...
  onSave() {
      if(this.id == -1) {
        ...
        this.service.addCustomer("test1", this.customer).subscribe(
          response => {
        ...
        this.service.updateCustomer("test1", this.customer).subscribe(
          response => {
            console.log(response);
            this.router.navigate(['customers']);
          }
